...
public class ShishangHomeFragment extends Fragment {
    @InjectView(R.id.act_recycler_variety_recycler_shishang)
    RecyclerView actRecyclerVarietyRecyclerShishang;
    @InjectView(R.id.refreshLayout_shishang)
    SmartRefreshLayout refreshLayoutShishang;
    RefreshLayout isrefreshLayout;
    boolean flag = false;
    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_shishanghome, container, false);
        ButterKnife.inject(this, view);
        refreshLayoutShishang.setOnRefreshListener(new OnRefreshListener() {
            @Override
        ...
        refreshLayoutShishang.setOnLoadMoreListener(new OnLoadMoreListener() {
            @Override
        ...
        refreshLayoutShishang.setRefreshHeader(new ClassicsHeader(getContext()));//.setEnableHorizontalDrag(true)
        //设置 Footer 为 球脉冲 样式
        refreshLayoutShishang.setRefreshFooter(new BallPulseFooter(getContext()).setSpinnerStyle(SpinnerStyle.Scale));
        getMessage();
        return view;
    }
    private void initViewOper(final List<HeadLineModel> headLineModelList) {
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
        actRecyclerVarietyRecyclerShishang.setLayoutManager(linearLayoutManager);
        actRecyclerVarietyRecyclerShishang.addItemDecoration(new DividerItemDecoration(getContext(), DividerItemDecoration.VERTICAL));
        HomeAdapter adapter = new HomeAdapter(headLineModelList, getContext());
        actRecyclerVarietyRecyclerShishang.setAdapter(adapter);
